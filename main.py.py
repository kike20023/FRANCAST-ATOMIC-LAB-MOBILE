...
# En navegador/Pygbag conviene usar una superficie simple. El propio canvas
# se encarga del escalado; RESIZABLE/SCALED pueden causar problemas en WASM.
if IS_WEB:
    flags = 0
else:
...
def sysfont(size, bold=False):
    # En web evitamos depender de fuentes del sistema que quizá no existan.
    if IS_WEB:
...
# ---------------------- LOOP PRINCIPAL ----------------------
async def main():
    global search, search_active, show_picker, picker_scroll, table_scroll
    new_question()
    running=True
    frame_count=0
    test_frames=int(os.environ.get("FRANCAST_TEST_FRAMES","0") or 0)
    while running:
        # En pygame-wasm el navegador ya sincroniza los frames. clock.tick()
        # puede bloquear el hilo WASM, así que solo lo usamos fuera de web.
        dt=(1.0/FPS) if IS_WEB else reloj.tick(FPS)/1000.0
